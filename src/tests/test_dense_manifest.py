"""Frozen-run dense PASTIS cache: slim sidecar, exact completeness, resumable, refuse-on-mismatch."""
...
def dense_env(tmp_path, monkeypatch):
    ...
    (tmp_path / "dd").mkdir()
    (tmp_path / "dd" / "pastis.txt").write_text("p" * 64)
...
def test_dense_roundtrip_slim_sidecar(dense_env):
    _tmp, bench = dense_env
    C.extract_dense_and_cache(bench, "pastis", "fakemodel", "baseline")
    m = C._read_manifest(C.dense_manifest_path("pastis", "fakemodel", "baseline"))
...
def test_dense_require_rejects_tampered_feature_dim(dense_env):
    _tmp, bench = dense_env
    C.extract_dense_and_cache(bench, "pastis", "fakemodel", "baseline")
    man = C.dense_manifest_path("pastis", "fakemodel", "baseline")
    m = C._read_manifest(man)
    m["feature_dim"] = 999
    C._write_manifest(man, m)
...
def test_dense_require_rejects_dataset_digest_change(dense_env):
    tmp, bench = dense_env
    C.extract_dense_and_cache(bench, "pastis", "fakemodel", "baseline")
    (tmp / "dd" / "pastis.txt").write_text("q" * 64)
...
def test_dense_extract_resumes_after_interruption(dense_env):
    _tmp, bench = dense_env
    root = C.extract_dense_and_cache(bench, "pastis", "fakemodel", "baseline")
    # Simulate a crash BEFORE the manifest, having lost one tile: absent manifest = incomplete.
    C.dense_manifest_path("pastis", "fakemodel", "baseline").unlink()
    (root / "fold_1" / "7_0_0.npy").unlink()
    (root / "fold_1" / "7_0_0.labels.npy").unlink()
    # Re-run resumes: re-encodes only the missing tile, then publishes the manifest.
    C.extract_dense_and_cache(bench, "pastis", "fakemodel", "baseline")
    C.require_dense_cache(bench, "pastis", "fakemodel", "baseline")
